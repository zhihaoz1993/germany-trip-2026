(() => {
  ...
  Object.assign(geo, {
    ...
    'Ruhestein, Baiersbronn, Germany': [48.571, 8.226],
    'Mummelsee, Seebach, Germany': [48.600, 8.202],
    'Caracalla Therme, Römerplatz 1, Baden-Baden, Germany': [48.761, 8.241],
  ...
  const sourceLabel = post => post.source || post.src || (post.url && post.url.includes('mafengwo') ? '马蜂窝' : post.url && post.url.includes('instagram') ? 'Instagram' : post.url && post.url.includes('youtube.com') ? 'YouTube' : post.url && post.url.includes('bilibili.com') ? 'Bilibili' : '小红书');
  const postCard = (post, index, dayImg) => {
    ...
    const isVideo = post.type === 'video' || /youtube\.com|bilibili\.com/.test(post.url || '');
    const badge = isVideo ? '<span class="video-badge">▶ 视频</span>' : '';
    ...
    return `<article class="xhs-post${isVideo ? ' xhs-video' : ''}"><a class="xhs-link" ${link}><div class="xhs-media"><div class="xhs-cover xhs-cover-fill">${badge}<b>${E(post.title)}</b><span>${E(post.likes || '推荐')} 赞 · ${E(label)}</span></div>${img}</div><div class="xhs-body"><div class="xhs-meta"><span>${E(label)}</span><span>${E(post.likes || '推荐')} 赞</span></div><b>${E(post.title)}</b><span>${E(post.takeaway)}</span></div></a></article>`;
  };
